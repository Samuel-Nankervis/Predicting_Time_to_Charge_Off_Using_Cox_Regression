...
::: {style="text-indent: 30px;"}
  ...
  Survival function: $S (t) = P(T > t), 0 < t < \infty$ This function represents the probability that an individual survives longer than a particular amount of time. For our data, that means the probability a loan has not been charged off after a certain amount of time.
   ...
   $\delta_i$: the censoring indicator, which is 0 if individual $i$ had the event and 1 if they were censored. When using R functions, we sometimes have to use 1 for having the event and 2 for censored, but to fit the equation we use 0 and 1. <!--The function I used required that censored data was equal to 1 and that uncensored data was equal to 2, instead of a standard 0 uncensored, 1 censored format I originally used to make the Is_Censored variable. Not sure if that messes up the theoretical formulas, so I left it as is. -->
...
<!--The notation you used here doesn't quite match what I found, should we change one or the other? -->
