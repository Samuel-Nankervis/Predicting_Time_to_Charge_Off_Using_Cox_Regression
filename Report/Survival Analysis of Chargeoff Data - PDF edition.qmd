...
#| attr-output: 'style="font-size: 0.1em;"'
#| class-output: "small"
head(Chargeoff_Data)
...
```{r}
#| label: Testing assumptions for Vif model 1
#| fig-width: 7
#| fig-height: 7
#| out-width: "100%"
#| out-height: "50%"
...
```{r}
#| label: Testing assumptions for Vif model 2
#| fig-width: 7
#| fig-height: 3
#| out-width: "100%"
...
p1 = ggplot(martChargeoff, aes(x = Monthly_Payment, y = mart_res)) +
...
martChargeoff = mutate(logChargeoff, martRes = mart_res)
# 3. Plot residuals against the predictor
p2 = ggplot(martChargeoff, aes(x = Effective_Annual_Interest_Rate, y = mart_res)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "loess") + # Adds a trend line
  theme_minimal() +
...
```{r}
#| label: model that takes term into account 1
#| fig-width: 12
#| fig-height: 12
#| out-width: "100%"
#| out-height: "70%"
...
```{r}
#| label: model that takes term into account 2
#| fig-width: 7
#| fig-height: 3
#| out-width: "100%"
FinalCoxMod2 = coxph(Surv(Time_to_Charge_Off, Is_Censored) ~ exp(Effective_Annual_Interest_Rate) + log(Time_To_Maturity) + strata(Application_Code), data = logChargeoffClean)
